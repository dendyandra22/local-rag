...
def ollama_first_stage(model_name, template_messages: list[dict], tools: list = None):
    tool_detail = {
        "tool_name": None,
        "args": None,
        "response": None,
    ...
    if response.message.tool_calls is not None:
        for tool in response.message.tool_calls:
            ...
            print(f"""-> Gemma 3 triggered {tool_detail["tool_name"]} call with arguments: {tool_detail["args"]}""")

    return tool_detail

def hf_first_stage(model, template_messages: list[dict], tools: list = None, tool_choice: dict = None):
    # tool_detail = {
    #     "tool_name": None,
    #     "args": None,
    #     "response": None,
    ...
    #       "description": "Finds data matching a combination of specific columns and values mentioned by users. CRITICAL: Only provide arguments that the user EXPLICITLY mentioned in their prompt. DO NOT guess, hallucinate, or assume these values if they aren't provided by the user.",
    #       "parameters": {
    #         "type": "object",
    #         "properties": {
    #           "args": {
    #             "type": "object",
    #             "description": "A key-value dictionary where keys are database column names and values are the exact search targets mentioned by the user. Example: {\"category\": \"Electronics\", \"status\": \"available\"}",
    ...
    print("RAG STAGING 1 HF!!!")
    print(response)

    return response

def hf_second_stage(model, template_messages: list[dict], tools: list = None):
    response = model.create_chat_completion(
        messages=template_messages,
        stream=True,
        max_tokens=None,
    ...
    print("RAG STAGING 2 HF!!!")
    print(response)

    return response

# def llama_tool_calling_handler()
